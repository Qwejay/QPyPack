...
**下拉菜单与样式优化**：修复非编辑模式下拉框导致的文本重叠Bug。
**按钮与布局规格统一**：统一“浏览”、“AST扫描”等工具按钮的长度和大小。
**细节提升**：拖拽区域的主标题与副标题均开启了自动换行，防止载入长路径文件时挤压主面板。
...
- **UI 与视觉重构**：
  - 重构 `QComboBox` 悬浮下拉框，统一极简白色主题与滚动体验。
  - Tab 标签栏改为居中对齐，优化窗口最小宽度（710px）。
- **打包稳定性提升**：
  - 依赖安装全流程接管，支持主/备源自动降级切源。
  - 补全 PyInstaller Windows 版本元数据的 `VarFileInfo` 翻译头字段。
  - 优化精简模式下的排除模块策略。
...
* **修复 pipreqs 崩溃**：尝试解决依赖自动扫描过程中的编码中断问题。
